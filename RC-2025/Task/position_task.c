...
pid_struct_t Dog2_speed_pid[4] = {0};
pid_struct_t Dog2_angle_pid[4] = {0}; // 为保持同步，选择使用受力最多电机角度环控四电机速度环

float Dog_target = 0.0f;
float Dog_target_lul = 0.0f;
float Dog_target_rul = 0.0f;
float Dog_target_ldl = 0.0f;
float Dog_target_rdl = 0.0f;

float Dog_target_lur = 0.0f;
float Dog_target_rur = 0.0f;
float Dog_target_ldr = 0.0f;
float Dog_target_rdr = 0.0f;
...
pid_struct_t Dog1_speed_pid[4] = {0};
pid_struct_t Dog1_angle_pid[4] = {0}; // 为保持同步，选择使用受力最多电机角度环控四电机速度环
...
{
	/*LU*/
	Dog_moto_1[0].total_angle = Dog_moto_1[0].round_cnt * 8192 + Dog_moto_1[0].angle - Read_angle[lul]; //
	Dog_moto_1[1].total_angle = Dog_moto_1[1].round_cnt * 8192 + Dog_moto_1[1].angle - Read_angle[lur]; //+ Read_angle[lur]
	/*LD*/
	Dog_moto_1[2].total_angle = Dog_moto_1[2].round_cnt * 8192 + Dog_moto_1[2].angle - Read_angle[ldl];
	Dog_moto_1[3].total_angle = Dog_moto_1[3].round_cnt * 8192 + Dog_moto_1[3].angle - Read_angle[ldr]; // 这里刚好对上
	/*RU*/
	Dog_moto_2[0].total_angle = Dog_moto_2[0].round_cnt * 8192 + Dog_moto_2[0].angle - Read_angle[rul];
	Dog_moto_2[1].total_angle = Dog_moto_2[1].round_cnt * 8192 + Dog_moto_2[1].angle - Read_angle[rur]; //
	/*RD*/
	Dog_moto_2[2].total_angle = Dog_moto_2[2].round_cnt * 8192 + Dog_moto_2[2].angle - Read_angle[rdl];
	Dog_moto_2[3].total_angle = Dog_moto_2[3].round_cnt * 8192 + Dog_moto_2[3].angle - Read_angle[rdr]; //
																										// if (Lift_Middle)
																										// {

	// Dog_target_lul = ABS(rc_ctrl.rc.ch[3]) / 660.0f * 140.0f;
	// Dog_target_lur = Dog_target_lul; // 电机本身的位置相反
	// if (rc_ctrl.rc.ch[4] > 0)
	// {
	// 	Dog_target_lul += 0.05;
	// 	Dog_target_lur = Dog_target_lul;
	// }
	...
	// }
	// }
	// limit_key(&Dog_target_lul, 140, 10);
	// limit_key(&Dog_target_lur, 140, 10);
	// limit_key(&Dog_target_rul, 140, 10);
	// limit_key(&Dog_target_rur, 140, 10);
	// limit_key(&Dog_target_ldl, 140, 10);
	// limit_key(&Dog_target_ldr, 140, 10);
	// limit_key(&Dog_target_rdl, 140, 10);
	// limit_key(&Dog_target_rdr, 140, 10);
...
void Dog_location(float Dog_target_lul, float Dog_target_rul, float Dog_target_ldl, float Dog_target_rdl, float Dog_target_lur, float Dog_target_rur, float Dog_target_ldr, float Dog_target_rdr) // 参数是编码值8192倍数
{
	// 一个足末端是一个目标值
	pid_calc(&Dog1_angle_pid[0], Dog_target_lul, Dog_moto_1[0].total_angle);
	pid_calc(&Dog1_angle_pid[1], Dog_target_lur, Dog_moto_1[1].total_angle);
	pid_calc(&Dog1_angle_pid[2], Dog_target_ldl, Dog_moto_1[2].total_angle);
	pid_calc(&Dog1_angle_pid[3], Dog_target_ldr, Dog_moto_1[3].total_angle);

	pid_calc(&Dog1_speed_pid[0], Dog1_angle_pid[0].output, Dog_moto_1[0].speed);
	pid_calc(&Dog1_speed_pid[1], Dog1_angle_pid[1].output, Dog_moto_1[1].speed);
	pid_calc(&Dog1_speed_pid[2], Dog1_angle_pid[2].output, Dog_moto_1[2].speed);
	pid_calc(&Dog1_speed_pid[3], Dog1_angle_pid[3].output, Dog_moto_1[3].speed);

	pid_calc(&Dog2_angle_pid[0], Dog_target_rul, Dog_moto_2[0].total_angle);
	pid_calc(&Dog2_angle_pid[1], Dog_target_rur, Dog_moto_2[1].total_angle);
	pid_calc(&Dog2_angle_pid[2], Dog_target_rdl, Dog_moto_2[2].total_angle);
	pid_calc(&Dog2_angle_pid[3], Dog_target_rdr, Dog_moto_2[3].total_angle);

	pid_calc(&Dog2_speed_pid[0], Dog2_angle_pid[0].output, Dog_moto_2[0].speed);
	pid_calc(&Dog2_speed_pid[1], Dog2_angle_pid[1].output, Dog_moto_2[1].speed);
	pid_calc(&Dog2_speed_pid[2], Dog2_angle_pid[2].output, Dog_moto_2[2].speed);
	pid_calc(&Dog2_speed_pid[3], Dog2_angle_pid[3].output, Dog_moto_2[3].speed);

	FOOTA_CURRENT(Dog1_speed_pid[0].output, Dog1_speed_pid[1].output, Dog1_speed_pid[2].output, Dog1_speed_pid[3].output);
	FOOTB_CURRENT(Dog2_speed_pid[0].output, Dog2_speed_pid[1].output, Dog2_speed_pid[2].output, Dog2_speed_pid[3].output);
}
