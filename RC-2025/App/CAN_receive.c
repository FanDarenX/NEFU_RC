...
void get_moto_measure(moto_measure *ptr, uint8_t RXdata[8])
{
	...
	//	ptr->total_angle_true = (ptr->total_angle + ptr->angle - ptr->last_angle) / 8192.0f / 19.0f * 360.0f;
}
/**************************************************************/
float ABS(float x)
{
	...
	if (HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &RX1message, RX1data) == HAL_OK)
	{
		if (hcan->Instance == CAN1)
		{
			...
			case CAN_3508M4_ID:
			{
				...
				get_moto_measure(&Dog_moto_1[j], RX1data);
				break;
			}
				/*这是机械臂吸盘两个电机*/
				// case CAN_2006M1_ID:
	...
	if (HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &RX2message, RX2data) == HAL_OK)
	{
		if (hcan->Instance == CAN2) // ID分配
		{
			...
			case CAN_3508MD_ID:
			{
				...
				get_moto_measure(&Dog_moto_2[j], RX2data);
				break;
			}
			/*这是龙门架纵向y轴电机*/
			// case CAN_2006LY_ID:
...
/*A电机电流发送*/
void FOOTA_CURRENT(int16_t K1, int16_t K2, int16_t K3, int16_t K4) // CAN1 ID:1~4
{
...
/*B电机电流发送*/
void FOOTB_CURRENT(int16_t K1, int16_t K2, int16_t K3, int16_t K4) // CAN2 ID:1~4
{
...
// void CAN_cmd_foota(int16_t motor1, int16_t motor2, int16_t motor3, int16_t motor4)
// {
// 	uint32_t send_mail_box;
// 	foota_tx_message.StdId = CAN_FOOT_ALLA_ID;
// 	foota_tx_message.IDE = CAN_ID_STD;
// 	foota_tx_message.RTR = CAN_RTR_DATA;
// 	foota_tx_message.DLC = 0x08;
// 	foota_can_send_data[0] = motor1 >> 8;
// 	foota_can_send_data[1] = motor1;
// 	foota_can_send_data[2] = motor2 >> 8;
// 	foota_can_send_data[3] = motor2;
// 	foota_can_send_data[4] = motor3 >> 8;
// 	foota_can_send_data[5] = motor3;
// 	foota_can_send_data[6] = motor4 >> 8;
// 	foota_can_send_data[7] = motor4;

// 	HAL_CAN_AddTxMessage(&FOOTA_CAN, &foota_tx_message, foota_can_send_data, &send_mail_box);
// }

// void CAN_cmd_footb(int16_t motor5, int16_t motor6, int16_t motor7, int16_t motor8)
// {
// 	uint32_t send_mail_box;
// 	footb_tx_message.StdId = CAN_FOOT_ALLB_ID;
// 	footb_tx_message.IDE = CAN_ID_STD;
// 	footb_tx_message.RTR = CAN_RTR_DATA;
// 	footb_tx_message.DLC = 0x08;
// 	footb_can_send_data[0] = (motor5 >> 8);
// 	footb_can_send_data[1] = motor5;
// 	footb_can_send_data[2] = (motor6 >> 8);
// 	footb_can_send_data[3] = motor6;
// 	footb_can_send_data[4] = (motor7 >> 8);
// 	footb_can_send_data[5] = motor7;
// 	footb_can_send_data[6] = (motor8 >> 8);
// 	footb_can_send_data[7] = motor8;
// 	HAL_CAN_AddTxMessage(&FOOTB_CAN, &footb_tx_message, footb_can_send_data, &send_mail_box);
// }
